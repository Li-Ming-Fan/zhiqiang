...
class DoubleDQN(torch.nn.Module, AbstractAgent):
    """
    """
    def __init__(self, settings, agent_modules, env=None, learning=True):
        ...
        self.check_necessary_elements(DoubleDQN)
        #
    ...
    def optimize(self, batch_std, buffer):
        ...
        #
        # decoupled
        p_av_action = self.qnet_learner.infer(p_std)        # [B, A]
        p_action = torch.argmax(p_av_action, -1)           # [B, ]
        #
        p_av_target = self.qnet_base.infer(p_std)        # [B, A]
        #
