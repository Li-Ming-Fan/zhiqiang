...
class PPO_ACV(torch.nn.Module, AbstractAgent):
    """ Proximal Policy Optimization, 近邻策略优化。
    1, proximal optimization for the actor (policy-net),
    2, a single network for the actor and the critic.

    """
    def __init__(self, settings, agent_modules, env=None, learning=True):
        """ A single qnet works as the policy-net and the V-value net. 
        """
        super(PPO_ACV, self).__init__()
        self.check_necessary_elements(PPO_ACV)
        #
    ...
    def optimize(self, batch_std, buffer):
        ...
        #
        # reg_pg
        loss_pg = - target * log_ap_l - loss_entropy * self.reg_entropy
