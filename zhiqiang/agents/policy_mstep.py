...
class MStepPolicy(AbstractAgent):
    ...
    def optimize(self, batch_std, buffer):
        ...
        s_av = self.pnet_learner.infer(s_std)
        indices = batch_std["a"].long().unsqueeze(-1)
        s_exe_av = torch.gather(s_av, 1, indices)   # [B, 1]
        #
        # reward
        reward = batch_std["r"]
        reward_with_baseline = torch.mean(reward) - reward     # [B, ]
        reward_with_baseline = reward_with_baseline.detach()
        #
        # loss
        loss_pg = reward_with_baseline * torch.log(s_exe_av.squeeze(-1))  # [B, ]
        loss_pg = torch.mean(loss_pg)
        #
        self.pnet_learner.back_propagate(loss_pg)
        #
